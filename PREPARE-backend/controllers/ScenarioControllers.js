...
module.exports = {
						...
						return ScenarioModel.traineeHistory(lname, function (err2, histDetails) {
							...
							return ScenarioModel.traineeHistorySkills(lname, function (err3, histSkillsDetails) {
								...
								return ScenarioModel.getConnectedDevices(params, function (err4, connectedDevices) {
									...
									return ScenarioModel.getSentimentDetails(params, function (errSentiment, sentimentDetails) {
										if (errSentiment) {
											return callback(errSentiment);
										}
										...
										return ScenarioModel.traineeHistorySpecificSkill(lname, function (err4, histSpecificSkillsDetails) {
											...
											return ScenarioModel.getUploadFileList(params, function (err6, vidList) {
												...
												return ScenarioModel.traineeHistoryEvent(lname, function (err5, histEventsDetails) {
													...
													output.sentimentDetails = sentimentDetails
	...
	changeModelVersion: function (inputdata, callback) {
		try {
			var insertParams = inputdata;
			return ScenarioModel.changeModelVersion(insertParams, function (err, results) {
				if (err) {
					console.log(err);
					throw err;
				}
	...
	saveAdjustedTimestamps: function (inputData, callback) {
		try {
			var adjustedTimeStampRows = [];

			inputData.forEach(function (playData) {
				var temp = [];
				temp.push(playData.adjustedTimestamp);
				temp.push(playData.play_id);
				temp.push(playData.event_id);
				adjustedTimeStampRows.push(temp);
			});

			return ScenarioModel.saveAdjustedTimestamps(adjustedTimeStampRows, function (err, results) {
				if (err) {
					throw err;
				}
				...
				var send = {};
				send.error = false;
				send.message = "Adjusted TimeStamps saved";
				return callback(null, send);
			});
	...
	//Save sentiment detection information to played_nlp_events table
	saveSentimentPlay: function (inputData, callback) {
		console.log(inputData)
		try {
			var sentimentPlayDataRows = [];

			inputData.forEach(function (sentimentData) {
				var temp = [];
				temp.push(sentimentData.play_id);
				temp.push(sentimentData.scenario_role_id);
				temp.push(sentimentData.scenario_id);
				temp.push(sentimentData.predicted_text);
				temp.push(sentimentData.predicted_sentiment);
				temp.push(sentimentData.timestamp);
				temp.push(sentimentData.model_version);

				sentimentPlayDataRows.push(temp);
			});

			return ScenarioModel.saveSentimentPlay(sentimentPlayDataRows, function (err, results) {
				if (err) {
					throw err;
				}
				...
				var send = {};
				send.error = false;
				send.message = "Sentiment Play Data Saved";
				return callback(null, send);
			});
	...
	getTrainingText: function (inputdata, callback) {
		try {
			return ScenarioModel.getTrainingText(inputdata, function (err1, scenarios) {
				console.log("Returned Scenarios:", scenarios);
				var data = {
					data: scenarios,
					message: 'Data Entered'
				}
