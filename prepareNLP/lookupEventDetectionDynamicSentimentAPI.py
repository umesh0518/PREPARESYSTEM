...
nodes = [] # arrays of values to be saved to played_nlp_events table
sentiment_nodes = [] # arrays of values to be saved to played_sentiment_events table
...
def process_prediction (lookup_dict,event_id,event_time,event_timeout,event_penalty_coefficient,scenario_name,scenario_id,model_name,scenario_role_id,selected_model_version):
    global final_predicted_event_set
    
    # Check if selected_model_version is provided
    if selected_model_version:
        model_path = os.path.join("Models", str(scenario_id), selected_model_version)
    else:
        # If not, fetch the path of the latest event detection model
        model_path = get_latest_model_path(scenario_id, model_name)
        
    print('This is the model Path',model_path)
    modelName_version = os.path.basename(model_path)
    print('Model name_version',modelName_version)
    # Load Event Model
    ...
    #Fetch the labels from the sentiment model
    ...
    while True: 
        ...
        if len(input_text) !=0:
            ...
            # Make sentiment prediction using the sentiment model
            ...
            print('Input Text:',input_text , "Sentiment :",predicted_sentiment)
            ...
            sentiment_dict = {
                ...
                'timestamp': str(int(time.time() * 1000)),
                'model_version': modelName_version
            }
            ...
            sentiment_nodes.append(sentiment_dict)
...
def predict_event():
    ...
    global endTimestamp
    global nodes
    global sentiment_nodes
    global scenario_duration
    # Reset nodes ans sentiment_nodes before start of processing
    nodes = []
    sentiment_nodes = []
    ...
    selected_model_version = request.json.get('selected_model_version')
    ...
    if t is None or not t.is_alive():
        t = threading.Thread(target=process_prediction, args = (lookup_dict,event_id,event_time,event_timeout,event_penalty_coefficient,scenario_name,scenario_id,model_name,scenario_role_id,selected_model_version))
        t.start()
...
def stop_processing():
    ...
    for sentiment_node in sentiment_nodes:
        sentiment_node['play_id'] = play_id
        
    endTimestamp = int(time.time() * 1000)
    print('nodes:',nodes)
    print('sentiment nodes:',sentiment_nodes)
    ...
    nlpResponse = requests.post('http://136.247.82.55:1339/saveNlpPlay', json={"data": nodes})
    print('This is the response from sevrer',nlpResponse) #response from the server
    sentimentResponse = requests.post('http://136.247.82.55:1339/saveSentimentPlay', json={"data": sentiment_nodes})
    print('This is the response from sevrer',sentimentResponse) #response from the server

    # Reset nodes and sentiment_nodes for the next batch of processing
    nodes = []
    sentiment_nodes = []
